...
export function PuzzleSelector({ puzzles, selectedId, onSelect, completedIds }: PuzzleSelectorProps) {
  if (puzzles.length <= 1) return null

  const groups = DIFFICULTY_ORDER
    .map(label => ({ label, puzzles: puzzles.filter(p => getDifficulty(p) === label) }))
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 20,
                    border: '2px solid',
                    borderColor: isSelected ? '#7c3aed' : isCompleted ? '#16a34a' : 'rgba(0,0,0,0.15)',
