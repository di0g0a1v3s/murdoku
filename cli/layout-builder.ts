...
export function buildRooms(theme: PuzzleTheme, seed: number, gridRows: number, gridCols: number): Room[] {
...
const OBJECT_OFFSETS: Record<ObjectKind, Coord[]> = {
  chair:     [{ row: 0, col: 0 }],
  plant:     [{ row: 0, col: 0 }],
  table:     [{ row: 0, col: 0 }, { row: 0, col: 1 }],
  bed:       [{ row: 0, col: 0 }, { row: 1, col: 0 }],
  bookshelf: [{ row: 0, col: 0 }],
  sofa:      [{ row: 0, col: 0 }, { row: 0, col: 1 }],
  fireplace: [{ row: 0, col: 0 }],
  counter:   [{ row: 0, col: 0 }, { row: 0, col: 1 }],
...
function touchesWall(cells: Coord[], room: Room, gridRows: number, gridCols: number): boolean {
  ...
  for (const cell of cells) {
    for (const n of [
      ...
      if (n.row < 0 || n.row >= gridRows || n.col < 0 || n.col >= gridCols) return true
      if (!roomKeys.has(`${n.row},${n.col}`)) return true
    }
  }
  return false
}

function hasFreeAdjacent(cells: Coord[], room: Room, usedCells: Set<string>, minFree: number): boolean {
  ...
  for (const cell of cells) {
    for (const n of [
      ...
      if (!room.cells.some(rc => rc.row === n.row && rc.col === n.col)) continue
      free.add(key)
    }
  }
...
export function buildLayout(theme: PuzzleTheme, seed: number, gridRows: number, gridCols: number): LayoutResult {
  ...
  for (let roomIdx = 0; roomIdx < rooms.length; roomIdx++) {
    ...
    const requiredTemplates = shuffle(OBJECT_TEMPLATES.filter(t => required.includes(t.kind)), rng)
    ...
    while (placed < numObjects) {
      ...
      for (const template of orderedTemplates) {
        ...
        for (const anchor of roomCells) {
          ...
          if (cellsInRoom(cells, room) && cellsNotOccupied(cells, usedCells) && hasFreeAdjacent(cells, room, usedCells, template.minFreeAdjacent) && (!template.mustTouchWall || touchesWall(cells, room, gridRows, gridCols))) {
      ...
      if (!placedOne) break // no template fits anywhere in this room — stop
    }
